...
export default function LoginChofer({ navigation, onLogin }) {
  ...
  const entrar = async () => {
    ...
    setEntrando(true);
    setError("");
    const r = await entrarSesion("chofer", correo, password);
    if (!r.ok) {
      setError(r.mensaje);
      setEntrando(false);
      return;
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1, backgroundColor: T.fondo }}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.chip}><Feather name="truck" size={13} color={T.tealClaro} /><Text style={s.chipTxt}>CHOFER / OPERADOR</Text></View>
        <Text style={s.h1}>Ruta del día</Text>
        <Text style={s.p}>Escanea el QR del contenedor y registra la recolección con foto antes y después.</Text>
        ...
        <TextInput style={s.input} placeholder="tu correo de chofer" placeholderTextColor={T.grisClaro} autoCapitalize="none" keyboardType="email-address" value={correo} onChangeText={setCorreo} />

        <Text style={s.label}>Contraseña</Text>
        <CampoClave style={s.input} value={password} onChangeText={setPassword} onSubmitEditing={entrar} />

        <Boton variante="teal" onPress={entrar} style={{ marginTop: 20 }}>Entrar</Boton>
...
const s = StyleSheet.create({
  ...
  chip: { flexDirection: "row", alignItems: "center", gap: 6, alignSelf: "flex-start", backgroundColor: "rgba(79,192,197,0.14)", borderColor: "rgba(79,192,197,0.5)", borderWidth: 1, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5, marginBottom: 10 },
  chipTxt: { color: T.tealClaro, fontSize: 11, fontWeight: "800", letterSpacing: 0.06 },
